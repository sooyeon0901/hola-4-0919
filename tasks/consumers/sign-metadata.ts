...
async function consume() {
  ...
  channel.assertExchange('delayedDeadLetterExchange', 'x-delayed-message', {
    autoDelete: false,
    durable: true,
  ...
  channel.assertExchange('deadLetterExchange', 'topic');
  ...
  channel.bindQueue('retryQueue', 'delayedDeadLetterExchange', 'dle-key');
  ...
  channel.consume(signingQueue, async function (msg) {
    if (msg !== null) {
      ...
      const { connection, keypair, endpoint } = await buildSolana(process.env.SOLANA_SECRET_ID)({
